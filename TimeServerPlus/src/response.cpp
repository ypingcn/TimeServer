...
int tsp_response_match(const tsp_request_t * request,const tsp_module_t * module)
{
  ...
  if(module->pattern.method != "" && request->method != module->pattern.method)
    return -1;
  if(module->pattern.version == std::string("") || module->pattern.version == std::string() )
  {
...
void tsp_response_date(int sockfd)
{
  ...
  string mons[] = {"Jan","Feb","Mar","Apr","May","June","July","Aug","Sept","Oct","Nov","Dec"};

  char status[1024];

  snprintf(status, sizeof(status), "Date: %s, %d %s %4d %02d:%02d:%02d GMT\r\n",
          wdays[gmt->tm_wday].data(),gmt->tm_mday,mons[gmt->tm_mon].data(),gmt->tm_year+1900,gmt->tm_hour,gmt->tm_min,gmt->tm_sec);
...
{
  char status[1024];
  snprintf(status,sizeof(status),"HTTP/1.1 %d %s\r\n", TSP_RESPONSE_FORBIDDEN,"Forbidden");
  write(sockfd,status,strlen(status));
}
void tsp_response_not_found(int sockfd)
{
  char status[1024];
  snprintf(status, sizeof(status), "HTTP/1.1 %d %s\r\n",TSP_RESPONSE_NOT_FOUND,"Not Found");
  write(sockfd,status,strlen(status));
}
...
{
  char status[1024];
  snprintf(status,sizeof(status),"HTTP/1.1 %d %s\r\n", TSP_RESPONSE_HTTP_VERSION_NOT_SUPPORTED,"Http Version Not Supported");
  write(sockfd,status,strlen(status));
}
