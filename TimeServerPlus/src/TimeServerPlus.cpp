...
int tsp_server_init(int port)
{
  ...
  tsp_socket_bind(server_socket_fd, (struct sockaddr*)&server_addr,sizeof(server_addr));
...
void tsp_server_event_circles(int server_fd)
{
  ...
  while(true)
  {
    ...
    for(int i = 0; i != num; i++)
    {
      ...
      {
        client_socket_fd = tsp_socket_accept(server_fd,(struct sockaddr*)&client_addr,&addrlen);
      ...
      else
      {
        ...
        pthread_t tid;
        pthread_create(&tid,&pthread_attr_detach,&tsp_server_thread_function,(void *)(intptr_t)client_socket_fd);
      }
    }
  }
