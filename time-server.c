...
int init(uint16_t port)
{
  ...
  int reuse_on = 1;
  if( setsockopt(httpd, SOL_SOCKET, SO_REUSEADDR, &reuse_on,sizeof(reuse_on)) < 0)
...
int main(int argc, char const *argv[])
{
  ...
  while(1)
  {
    ...
    if( pthread_create(&new_pthread, NULL, (void *)accept_request, (void *)(intptr_t)client_socket ) < 0)
